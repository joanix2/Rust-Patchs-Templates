...
pub fn merge_patch(
    ...
    let mut base_map: std::collections::HashMap<String, Item> = base_items
        .iter()
        .filter_map(|item| {
            extract_item_name(item).map(|name| (name, item.clone()))
        })
        .collect();
    ...
    for op in &patch.operations {
        match op {
                ...
                if base_map.contains_key(name) {
                    match strategy {
                        ...
                        MergeStrategy::PreferManual => {
                            ...
                            conflicts.push(format!("Item '{}' exists in both base and patch", name));
                        }
                        MergeStrategy::FailOnConflict => {
                            conflicts.push(format!("Conflict: Item '{}' exists in both base and patch", name));
                        }
                    }
                } else {
                    merged_items.push(item.clone());
                ...
                if let Some(base_item) = base_map.get(name) {
                    ...
                    match strategy {
                        ...
                        MergeStrategy::PreferManual => {
                            ...
                            base_map.remove(name);
                            conflicts.push(format!("Item '{}' was deleted in template but exists in base", name));
                        }
                        MergeStrategy::FailOnConflict => {
                            conflicts.push(format!("Conflict: Item '{}' was deleted in template but modified in base", name));
                        }
                    }
                } else {
            ...
            PatchOp::Modify { name, old_item: _, new_item } => {
                ...
                if let Some(base_item) = base_map.remove(name) {
                    ...
                    if base_code == new_code {
                        ...
                        match strategy {
                            MergeStrategy::PreferTemplate => {
                                merged_items.push(new_item.clone());
                                conflicts.push(format!("Item '{}' has manual changes, overridden by template", name));
                            }
                            MergeStrategy::PreferManual => {
                                merged_items.push(base_item);
                                conflicts.push(format!("Item '{}' has manual changes, template update skipped", name));
                            }
                            MergeStrategy::FailOnConflict => {
...
mod tests {
    ...
    fn test_merge_keep_manual_additions() {
        let base_items: Vec<Item> = vec![
            parse_quote! { fn manual_fn() {} }
        ];
        let new_items: Vec<Item> = vec![
            parse_quote! { fn template_fn() {} }
        ];
